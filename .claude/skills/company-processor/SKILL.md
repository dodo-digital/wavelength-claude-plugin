...
4. **Validate emails** [LOW freedom]
   **Primary (MCP):** Use the Wavelength MCP tools if available:
   - `mcp__wavelength__validate_email` — Clearout validation (one email at a time)
   - `mcp__wavelength__zb_validate_email` — ZeroBounce validation (one email at a time)
   - `mcp__wavelength__bulk_validate` — Clearout bulk validation (preferred for large lists)
   - `mcp__wavelength__check_credits` — verify remaining credits before a run

   Map results to `clearout_rating` and `zerobounce_rating` columns.

   **Fallback (offline):** If MCP tools are unavailable, extract all emails to a temp file (one per line), then run:
   ```
   ...
   **If neither MCP nor script is available:** Skip validation, warn the user that emails are unvalidated.
